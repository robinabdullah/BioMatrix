    ...
    public partial class Master : Form
    {
        ...
        private void btnPullData_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var lstMachineInfoNew = new List<Data>();
                foreach (var item in lstMachineInfo)
                {
                    if(item.DateOnlyRecord.Date >= DateTime.Now.Date)
                    {
                        lstMachineInfoNew.Add(new Data() { Id = item.IndRegID, EntryTime = item.DateTimeRecord });
                    }
                }
